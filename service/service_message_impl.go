...
func (service_message *ServiceMessageImpl) Create(ctx context.Context, request web.RequestMessage) web.ResponseMessage {
	...
	return helper.ResponseMessage(message)
...
func (service_message *ServiceMessageImpl) SelectAllSenderWithLastMessage(ctx context.Context) []web.ResponseMessage {
	db := service_message.DB
	message := service_message.RepositoryMessage.SelectAllSenderWithLastMessage(ctx, db)
