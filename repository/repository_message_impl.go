...
func (repository_message *RepositoryMessageImpl) Create(ctx context.Context, DB *sql.DB, message domain.Massage) domain.Massage {
	sql := "Insert Into message(sender,date,message,receiver) Values (?,?,?,?)"
	result, err := DB.ExecContext(ctx, sql, message.Sender, message.Date, message.Message, message.Receiver)
...
func (repository_message *RepositoryMessageImpl) SelectAllSenderWithLastMessage(ctx context.Context, DB *sql.DB) []domain.Massage {
	//sql := "Select id,sender,date,message,receiver from message Where receiver=? && id > ?"
	sql := "select max(m.id), m.sender,m.message, m.receiver, count(*)-1 as jumlah from message m left join last_read lr on m.sender_id = lr.sender_id where m.id >= lr.message_id group by m.sender_id"
